...
const ProgressTrackingScreen: React.FC<ProgressTrackingScreenProps> = ({ onClose, onTabChange, onAddPress, currentTab = 'diario' }) => {
  ...
  useEffect(() => {
    loadProgressData();
  }, [selectedChart, selectedTimeFilter]);
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={styles.chartArea}>
          ...
          <View style={[styles.chart, { width: chartWidth, height: chartHeight }]}>
            ...
            {/* Puntos de datos */}
            {data.map((point, index) => {
              const y = chartPadding + (chartHeight - chartPadding * 2) - ((point.value - minValue) / range) * (chartHeight - chartPadding * 2);
              ...
              return (
                <TouchableOpacity key={index} style={[styles.chartPoint, { left: x - 6, top: y - 6 }]}>
                  <View 
                    style={[
                      styles.chartDot,
                      {
                  ...
                  />
                  <Text style={[styles.xAxisLabel, { left: labelLeft, bottom: -25 }]}>{point.date}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
