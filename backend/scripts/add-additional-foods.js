...
async function addAdditionalFoods() {
  try {
    ...
    // Solo añadir si hay menos de 50 alimentos
    if (currentCount >= 50) {
      console.log('ℹ️ Ya hay suficientes alimentos en la base de datos');
      return;
    }
    ...
    console.log(`📝 Preparando ${foodsToAdd.length} alimentos adicionales para insertar...`);
    ...
    console.error('❌ Error durante la adición de alimentos:', error);
    throw error;
  }
}
...
if (require.main === module) {
  addAdditionalFoods()
    .then(() => {
      console.log('✅ Script de alimentos adicionales completado');
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = addAdditionalFoods;
