...
interface SimpleAffiliateStats {
...
interface SimpleAffiliateDashboardProps {
  onClose: () => void;
}

export const SimpleAffiliateDashboard: React.FC<SimpleAffiliateDashboardProps> = ({ onClose }) => {
  const [stats, setStats] = useState<SimpleAffiliateStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const fetchStats = async () => {
    try {
      console.log('🔄 [SIMPLE DASHBOARD] Obteniendo estadísticas...');
      const response = await affiliateApiService.getAffiliateDashboard();
      console.log('✅ [SIMPLE DASHBOARD] Respuesta recibida:', response);
      
      setStats(response);
    } catch (error) {
      console.error('❌ [SIMPLE DASHBOARD] Error obteniendo estadísticas:', error);
  ...
  console.log('✅ [SIMPLE DASHBOARD] Renderizando con stats:', stats);
