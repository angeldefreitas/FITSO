...
const ProgressTracking: React.FC<ProgressTrackingProps> = ({
  ...
  const loadCaloriesData = async () => {
    if (!user?.id) {
      console.log('❌ No hay userId para cargar datos de calorías');
      return;
    }
    
    try {
      const dateKey = selectedDate.toISOString().split('T')[0];
      const savedData = await AsyncStorage.getItem(`caloriesData_${user.id}_${dateKey}`);
  ...
  const saveCaloriesData = async (newData: CaloriesData) => {
    if (!user?.id) {
      console.log('❌ No hay userId para guardar datos de calorías');
      return;
    }
    ...
    try {
      ...
      await AsyncStorage.setItem(`caloriesData_${user.id}_${dateKey}`, JSON.stringify(dataWithDate));
        ...
        <View style={styles.chartArea}>
          ...
          <View style={[styles.chart, { width: chartWidth, height: chartHeight }]}>
            ...
            <View style={[styles.lineContainer, { width: chartWidth, height: chartHeight }]}>
              {data.length === 1 ? (
                ...
                data.map((point, index) => {
                  ...
                  const currentY = chartPadding + effectiveHeight - ((point.value - minValue) / range) * effectiveHeight;
                  const prevY = chartPadding + effectiveHeight - ((prevPoint.value - minValue) / range) * effectiveHeight;
                  const currentX = chartPadding + ((index / (data.length - 1)) * effectiveWidth);
                  const prevX = chartPadding + (((index - 1) / (data.length - 1)) * effectiveWidth);
            ...
            {data.map((point, index) => {
              ...
              const x = chartPadding + ((index / (data.length - 1)) * effectiveWidth);
